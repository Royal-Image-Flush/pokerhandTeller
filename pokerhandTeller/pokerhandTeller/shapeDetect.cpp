...
void Card::preprocess()
{
	...
	//resize(img_tf, img_tf, Size(0, 0), 3, 3, INTER_LINEAR);

	// contours
	vector<vector<Point> > contours;
	findContours(img_tf, contours, RETR_LIST, CHAIN_APPROX_SIMPLE);

	// suit
	Rect rect = boundingRect(contours[0]);
	contours.erase(contours.begin());
	this->img_suit = img_tf(rect);

	// number
	sort(contours.begin(), contours.end(), cmp_contour);
	rect = boundingRect(contours[0]);
	this->img_num = img_tf(rect);
}
...
void Card::match()
{
	...
	for (int i = 0; i < sizeof(::SUIT) / sizeof(::SUIT[0]); i++) {
		...
		Mat img_train_suit = imread("./suit image/" + SUIT[i] + ".jpg");
